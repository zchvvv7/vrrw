...
class CorridorPredictor(CorridorPredictorInterface):
    ...
    def __init__(self, config: Optional[dict] = None) -> None:
        ...
        self._method = str(config.get("method", "road_mask_geometry"))
        self._model_version = str(config.get("model_version", "v1.0"))
        self._min_road_width = int(config.get("min_road_width", 50))
        self._temporal_window = int(config.get("temporal_window", 5))
        self._confidence_decay = float(config.get("confidence_decay", 0.9))
        self._boundary_sample_step = int(config.get("boundary_sample_step", 2))
        self._min_valid_rows = int(config.get("min_valid_rows", 10))
        self._corridor_width_ratio = float(
            config.get("corridor_width_ratio", 0.5)
        )
        self._corridor_top_ratio = float(config.get("corridor_top_ratio", 0.8))
        ...
        self._width_risk_weight = float(config.get("width_risk_weight", 0.3))
        self._history_buffer: Deque[dict] = deque(maxlen=MAX_HISTORY_SIZE)
        ...
        # 新增：自车速度（米/秒），用于计算时间
        self._ego_speed_mps = float(config.get("ego_speed_mps", 13.9))
    ...
    def predict(
        ...
        try:
            ...
            # 新增：应用障碍物避让逻辑，缓存结果
            self._last_obstacle_cutout_mask = self._apply_obstacle_avoidance(
                corridor_mask, known_objects, unknown_regions
            )

            # 新增：计算预测信息，缓存结果
            frame_height = frame.shape[0]
            self._last_prediction_markers = self._compute_prediction_markers(
    ...
    def _predict_corridor(
        ...
        np.ndarray, List[Tuple[int, int]], List[Tuple[int, int]], float
    ]:
        ...
        if len(valid_rows) < self._min_valid_rows:
            ...
            self._update_history(empty_mask, [], [], 0.0)
        ...
        corridor_mask = self._build_corridor_mask(polygon, height, width)

        _, direction_risk = self._analyze_direction(centerline, width)

        geometry_risk = self._estimate_geometry_risk(valid_rows, height)

        width_risk = self._estimate_width_risk(corridor_mask, road_mask)

        raw_confidence = self._compute_confidence(
            geometry_risk, direction_risk, width_risk
        )

        smoothed_confidence = self._temporal_smooth_confidence(raw_confidence)
    ...
    def _interpolate_boundaries(
        ...
        all_rows = sorted(set(range(0, height, self._boundary_sample_step)))
        ...
        for y in all_rows:
            ...
            if prev_valid is not None and next_valid is not None:
                ratio = (y - prev_valid) / (next_valid - prev_valid)
                val = int(
    ...
    def _build_trapezoid_corridor(
        ...
        Tuple[dict, dict, List[Tuple[int, int]], List[Tuple[int, int]]]
    ]:
        ...
        if vp_y >= road_bottom_y:
            ...
            actual_bl = float(left_boundary.get(actual_bottom_y, 0))
            actual_br = float(right_boundary.get(actual_bottom_y, width - 1))
            ...
            left_b = (ego_center_x - half_w_raw) - left_a * road_bottom_y
            right_b = (ego_center_x + half_w_raw) - right_a * road_bottom_y
        else:
        ...
        trap_top_y = int(
            trap_bottom_y + (vp_y - trap_bottom_y) * self._corridor_top_ratio
        )
        # 不超过道路实际可见范围
        ...
        road_bottom_width = road_bottom_right - road_bottom_left
        ...
        trap_bottom_left = ego_center_x - (road_bottom_width * scale) / 2.0
        trap_bottom_right = ego_center_x + (road_bottom_width * scale) / 2.0

        # 梯形左边缘平行于道路左边缘（斜率 left_a），起点在底边左下角
        trap_left_b = trap_bottom_left - left_a * trap_bottom_y
        # 梯形右边缘平行于道路右边缘（斜率 right_a），起点在底边右下角
        trap_right_b = trap_bottom_right - right_a * trap_bottom_y
        ...
        if num_points >= 2:
            for i in range(num_points):
                t = i / (num_points - 1)
                y = int(trap_bottom_y + (trap_top_y - trap_bottom_y) * t)
                cx = (
                    left_a * y + trap_left_b + right_a * y + trap_right_b
                ) / 2.0
    ...
    def _analyze_direction(
        ...
        if abs(shift) > width * 0.02:
            if shift > 0:
                ...
                direction = "left"
            direction_risk = min(1.0, abs(shift) / (width * 0.1))
    ...
    def _temporal_smooth_confidence(
        ...
        recent_confidences = [h["confidence"] for h in self._history_buffer]

        weights = [
            self._confidence_decay**i for i in range(len(recent_confidences))
        ]
        weighted_sum = sum(
            c * w for c, w in zip(reversed(recent_confidences), weights)
        )
        weight_total = sum(weights)
        historical_avg = (
            weighted_sum / weight_total if weight_total > 0 else raw_confidence
        )

        smoothed = 0.6 * raw_confidence + 0.4 * historical_avg
