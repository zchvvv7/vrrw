...
class RiskEvaluator:
    ...
    def __init__(
        ...
        self._history_size = int(
            config.get("history_size", 5)
        )
        self._confirm_frames = int(
            config.get("confirm_frames", 3)
        )
        self._release_frames = int(
            config.get("release_frames", 5)
        )
        ...
        self._current_reason = "no_obstacle"
    ...
    def evaluate(
        ...
        try:
            ...
            if (
                system_status == SystemStatus.DEGRADED
                and RISK_ORDER[raw_level]
                < RISK_ORDER["notice"]
            ):
                raw_level = "notice"
                raw_reason = "perception_degraded"
            risk_level, major_reason = self._apply_hysteresis(
                raw_level,
                raw_reason,
            )
            inference_time_ms = (
    ...
    def _compute_spatial_relation(
        ...
        if overlap >= self._intersection_ratio:
    ...
    def _estimate_ttc(
        ...
        if np.ptp(times) <= 0.0:
            return None
        slope = float(np.polyfit(times, distances, 1)[0])
        closing_speed = -slope
        if closing_speed < self._min_closing_speed_mps:
            return None
    ...
    def _classify_obstacle(
        ...
        if relation == "intersecting":
            if danger_by_distance or danger_by_ttc:
                return "danger", (
                    f"{source}_obstacle_intersecting_critical"
                )
            if warning_by_distance or warning_by_ttc:
                return "warning", (
                    f"{source}_obstacle_intersecting_close"
                )
    ...
    def _apply_hysteresis(
        ...
        raw_order = RISK_ORDER[raw_level]
        current_order = RISK_ORDER[self._current_level]
        if raw_order >= current_order:
            self._current_level = raw_level
            self._current_reason = raw_reason
            self._release_count = 0
            return self._current_level, self._current_reason
